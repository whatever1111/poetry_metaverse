...
- 完成状态：⏳ 待开始
- 执行步骤：
  - [ ] 步骤D.3.1：**最终提示词确认** - 与项目负责人最终确认新版提示词的每一个细节。
  - [ ] 步骤D.3.2：**代码实现** - 将最终版提示词替换到 `server.js` 的 `/api/interpret` 路由中。
  - [ ] 步骤D.3.3：**功能对比测试** - 选取至少3个有代表性的“用户特质-诗歌”组合，分别用旧版和新版提示词进行测试，记录并对比结果。
  - [ ] 步骤D.3.4：**结果分析与微调** - 分析测试结果，根据需要对提示词进行最后的微调。
  - [ ] 步骤D.3.5：**文档更新** - 确保代码中的注释或相关文档（如有）也更新了关于提示词的说明。
...
- [ ] D.3: AI解诗功能提示词优化 - 统一陆家明诗人语言风格 - 待开始
- [ ] D.4及后续UI细节优化任务待定义
