...
**任务列表**:
- [ ] **后台架构重设**: 设计一个以“宇宙仪表盘”为核心的后台主页，用于展示、创建和选择不同的宇宙 (`Universe`)。
- [ ] **Admin API 重构**:
    - [ ] 创建 `GET/POST/PUT/DELETE /api/admin/universes` 接口，用于增删改查 `Universe` 实体本身。
    - [ ] 设计一套动态路由机制，能根据所选宇宙的 `type`，加载并委托给该宇宙专属的管理API模块。
- [ ] **后台前端模块化**:
    - [ ] 将现有 `admin.html` 的功能拆分为一个“周与春秋宇宙管理模块”。
    - [ ] 当管理员在仪表盘选择“周与春秋”宇宙时，动态加载并显示此模块。
    - [ ] 为“毛小豆宇宙”创建一个占位符模块，并设计其初步的管理界面（如角色、场景列表）。
- [ ] **数据校验升级**: 将数据校验功能改造为可在“宇宙仪表盘”触发，可选择对特定宇宙或所有宇宙进行全局校验。

**退出条件**:
- [ ] 管理员可以通过后台界面，完成对 `Universe` 表的增删改查。
- [ ] 后台首页是一个“宇宙仪表盘”，清晰列出所有已创建的宇宙。
- [ ] 点击“周与春秋”宇宙，能够加载并使用其专属的管理功能（即原有的后台功能）。
- [ ] 为其他宇宙（如毛小豆）预留了清晰的管理模块扩展入口。
...
**任务列表**:
- [ ] **官网门户设计**: 将 `index.html` 重新设计为一个“陆家花园元宇宙”的门户页面，展示各个宇宙的入口，而非直接混合内容。
- [ ] **Public API 重构**:
    - [ ] 废弃 `/api/projects` 等扁平化接口，重构为宇宙中心的层级式接口，如：
        - `GET /api/universes`：获取所有公开可用的宇宙列表。
        - `GET /api/universes/:universeCode/content`：获取特定宇宙的核心内容（具体内容结构由该宇宙的 `type` 决定）。
    - [ ] 为“周与春秋”宇宙编写新的内容获取API，并更新其前端页面 (`index.html` 的旧逻辑) 以消费新API。
- [ ] **前端路由与组件化**:
    - [ ] 设计简单的客户端路由或多页面策略，实现 `/` (门户) -> `/zhou` (周与春秋) -> `/maoxiaodou` (毛小豆) 的导航。
    - [ ] 确保跨宇宙的通用组件（如顶栏、底栏）风格统一。

**退出条件**:
- [ ] 网站主页成为一个清晰的“宇宙门户”。
- [ ] 公开API以宇宙为中心进行重构，并有文档记录。
- [ ] 用户可以从门户页导航至“周与春秋”宇宙，其功能与体验和重构前保持一致。
...
**任务列表**:
- [ ] **端到端流程验证**:
    - [ ] 验证管理员工作流：登录 -> 宇宙仪表盘 -> 创建/编辑宇宙 -> 进入特定宇宙的管理模块。
    - [ ] 验证用户工作流：访问门户 -> 浏览宇宙列表 -> 进入特定宇宙体验。
- [ ] **新 API 契约测试**: 为所有新增和重构的 API 端点（管理和公开）编写合同测试，确保其行为符合预期。
- [ ] **文档完善**: 更新 `documentation/backend/api-contracts.md`，使其完全反映新的、以宇宙为中心的 API 结构。
- [ ] **系统稳定性测试**: 对动态模块加载、错误处理、边界条件（如空宇宙、未知宇宙类型）进行测试。

**退出条件**:
- [ ] 新架构下的前后端功能完整对接，核心工作流无阻塞。
- [ ] 关键 API 都有自动化测试覆盖，且文档清晰、同步。
- [ ] “周与春秋”宇宙在新的架构内功能稳定，与重构前体验一致，证明新架构的向后兼容能力。
