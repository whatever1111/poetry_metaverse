...
#### **B-0 前置条件：完成A-5环境配置**
> **重要**: 在开始B阶段工作前，必须先完成A-5的Prisma环境配置，确保数据库环境就绪。

> **🔍 问题发现**: 由于 `.gitignore` 文件配置，`lugarden_universal/application/prisma/` 和 `lugarden_universal/application/data/` 目录被排除在版本控制之外。虽然A-5阶段已完成配置，但这些文件未提交到Git，需要在新环境中重新配置。
...
- [ ] **任务 B-1.1**：分析现有JSON数据结构，制定迁移策略
  - [ ] 详细分析 `poeject_maoxiaodou_universe/data/` 中的9个JSON文件结构
  - [ ] 详细分析 `poeject_zhou_spring_autumn/data/content/` 中的JSON文件结构
  - [ ] 制定数据清洗和转换规则（处理空值、格式统一、ID映射等）
  - [ ] 确定迁移顺序（先主宇宙表，再子宇宙表，最后桥表）

- [ ] **任务 B-1.2**：创建数据迁移工具和脚本
  - [ ] 在 `lugarden_universal/application/` 创建 `scripts/migration/` 目录
  - [ ] 创建通用数据加载器 `scripts/migration/data-loader.js`
  - [ ] 创建毛小豆宇宙迁移脚本 `scripts/migration/migrate-maoxiaodou.js`
  - [ ] 创建周与春秋迁移脚本 `scripts/migration/migrate-zhou.js`
  - [ ] 创建主宇宙数据迁移脚本 `scripts/migration/migrate-main-universe.js`
  - [ ] 创建迁移执行器 `scripts/migration/run-migration.js`

- [ ] **任务 B-1.3**：执行数据迁移（分步骤验证）
  - [ ] 迁移主宇宙核心数据（Theme、Emotion、Universe表）
  - [ ] 迁移毛小豆宇宙数据（11张表）
  - [ ] 迁移周与春秋宇宙数据（5张表）
  - [ ] 建立桥表关联（UniverseTheme、UniverseEmotion）
  - [ ] 验证数据完整性（记录数、关键字段、关联关系）

#### **B-2 主宇宙数据映射关联建立**
- [ ] **任务 B-2.1**：建立主题映射关联
  - [ ] 分析毛小豆宇宙的6个主题与主宇宙Theme表的映射关系
  - [ ] 分析周与春秋诗歌的核心主题与主宇宙Theme表的映射关系
  - [ ] 创建UniverseTheme桥表数据，建立跨宇宙主题关联
  - [ ] 验证主题映射的完整性和准确性

- [ ] **任务 B-2.2**：建立情感映射关联
  - [ ] 从毛小豆宇宙的诗歌和场景中提取情感标签
  - [ ] 从周与春秋的问答系统中提取情感维度
  - [ ] 创建UniverseEmotion桥表数据，建立跨宇宙情感关联
  - [ ] 验证情感映射的合理性和覆盖度

- [ ] **任务 B-2.3**：建立跨宇宙内容关联
  - [ ] 通过诗歌标题建立毛小豆诗歌与周与春秋映射的关联
  - [ ] 通过主题关键词建立内容相似性关联
  - [ ] 创建跨宇宙内容推荐的基础数据结构
  - [ ] 验证跨宇宙关联的准确性和实用性
...
#### **B-3 数据完整性验证与一致性检查**
- [ ] **任务 B-3.1**：创建数据验证工具
  - [ ] 创建数据完整性检查脚本 `scripts/validation/check-integrity.js`
  - [ ] 创建关联关系验证脚本 `scripts/validation/check-relationships.js`
  - [ ] 创建数据一致性检查脚本 `scripts/validation/check-consistency.js`
  - [ ] 创建验证报告生成器 `scripts/validation/generate-report.js`
...
- [ ] **任务 B-3.3**：数据质量评估与修复
  - [ ] 生成数据质量报告，识别问题点
  - [ ] 修复发现的数据问题（空值、格式错误、关联缺失等）
  - [ ] 重新验证修复后的数据完整性
  - [ ] 建立数据质量监控基线

#### **B-4 数据统计与分析功能开发**
- [ ] **任务 B-4.1**：创建基础统计功能
  - [ ] 创建诗歌统计功能（按宇宙、主题、情感分类统计）
  - [ ] 创建角色统计功能（毛小豆宇宙角色关系网络分析）
  - [ ] 创建主题统计功能（跨宇宙主题分布和热度分析）
  - [ ] 创建时间线统计功能（毛小豆宇宙事件时间分布）

- [ ] **任务 B-4.2**：创建高级分析功能
  - [ ] 创建跨宇宙内容相似性分析
  - [ ] 创建用户行为模式分析（基于周与春秋问答数据）
  - [ ] 创建内容推荐算法基础
  - [ ] 创建数据可视化接口
...
## 重要提醒
在开始B阶段工作前，必须先完成B-0任务，确保Prisma环境配置正确。当前文档显示A-5已完成，但实际检查发现Prisma环境未完全配置的原因是：

### 🔍 **问题根源**
- **`.gitignore` 配置**: `lugarden_universal/application/prisma/` 和 `lugarden_universal/application/data/` 被排除在版本控制之外
- **设计意图**: 这是正确的，因为：
  - `prisma/` 目录包含生成的客户端代码，应该被忽略
  - `data/` 目录包含本地数据库文件，应该被忽略
  - 但 `schema.prisma` 设计文件应该被版本控制

### ✅ **解决方案**
- 在新环境中重新执行A-5的配置步骤
- 基于 `documentation/database/schema.md` 创建完整的 `schema.prisma` 文件
- 确保环境配置的完整性和可重现性

这个发现帮助我们理解了为什么需要B-0前置条件，确保阶段B的设计与实际工作流程完全一致。
