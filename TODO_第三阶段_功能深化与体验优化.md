...
- [ ] **任务A.5：BUG修复及功能新增**
  ...
  ##### - [ ] **步骤 A.5.5**: 古典回响页面设计与实现。
    ...
    - **预期改动（预判）**:
      ...
      - `lugarden_universal/public/assets/zhou.js` (修改showResult函数，新增古典回响页面逻辑)
    - **具体改进**: 
      - 新增古典回响页面，展示古典文献背景和智慧
      - 修改页面流程：showResult() → showClassicalEcho() → showPoemResult()
      - 设计优雅的古典回响页面布局和样式
      - 实现古典内容的解析和展示逻辑
      - 添加"当下的你也不孤独，吴任几是你的同行者"过渡按钮
      - 保持与现有设计系统的完全一致性
    - **页面设计**: 
      - 标题：简洁的"回响"
      - 副标题："你选择的道路，有古人智慧的回响"
      ...
      - 使用毛玻璃效果和优雅的动画
    - **技术实现**: 
      - 从`state.poemArchetypes`获取`classicalEcho`数据
      - 解析古典内容，分离文献名、原文和现代解读
      - 使用现有的页面切换动画系统
      - 保持响应式设计和移动端适配
      - 实现优雅降级（无古典数据时直接跳转诗歌页面）
    - **验收标准**: 
