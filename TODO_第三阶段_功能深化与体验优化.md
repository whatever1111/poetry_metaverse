...
- [ ] **任务A.5：BUG修复及功能新增**
  ...
  ##### - [ ] **步骤 A.5.5**: 古典回响页面设计与实现。
    ...
    ##### - [ ] **步骤 A.5.5.2**: 古典内容解析与展示逻辑实现。
      - **问题背景**: 需要实现`classicalEcho`字段的解析和展示，让古典内容能够正确显示
      - **问题分析**: 
        - `classicalEcho`字段包含复杂的古典内容信息
        - 需要解析文献名、原文、现代解读等不同部分
        - 需要处理不同格式的古典内容
        - 需要实现优雅降级（无古典数据时的处理）
      - **解决方案**: 实现古典内容的解析和展示逻辑
      - **预期改动（预判）**:
        - `lugarden_universal/public/assets/zhou.js` (新增古典内容解析函数)
      - **具体改进**: 
        - 实现`parseClassicalEcho()`函数，解析古典内容
        - 分离古典文献名、原文、现代解读等部分
        - 处理不同格式的古典内容（《易经》、《春秋公羊传》、《孟子》等）
        - 实现内容格式化，确保显示效果优雅
        - 添加错误处理和优雅降级机制
        - 从`state.poemArchetypes`获取当前诗歌的古典数据
      - **解析逻辑设计**: 
        - 文献名提取：识别《》符号内的文献名
        - 原文提取：识别古典原文内容
        - 现代解读：提取现代语言解读部分
