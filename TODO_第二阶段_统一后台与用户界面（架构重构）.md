...
- [ ] **任务A.1：实现 `Universe` 实体的基础 CRUD 管理**
  - **核心思想**: 严格按照 `任务 A.0` 中冻结的契约，完成后端接口和前端界面的开发。
  ...
  - [ ] **步骤 A.1.5:** 将 `admin.html` 的主体部分重构为“宇宙仪表盘”布局。
    - **预期改动（预判）**:
      - `lugarden_universal/public/admin.html` (修改 HTML 结构，为宇宙列表和管理入口做准备)
  - [ ] **步骤 A.1.6:** 在仪表盘前端，调用 `GET` 接口，获取并渲染宇宙列表。
    - **预期改动（预判）**:
      - `lugarden_universal/public/admin.html` (编写 JS 逻辑，发起 fetch 请求并动态创建列表元素)
  - [ ] **步骤 A.1.7:** 创建并集成一个可复用的模态框（Modal）表单，用于“创建新宇宙”和“编辑宇宙”。
    - **预期改动（预判）**:
      - `lugarden_universal/public/admin.html` (新增模态框的 HTML 结构和对应的显示/隐藏 JS 逻辑)
  - [ ] **步骤 A.1.8:** 为列表中的每个宇宙条目，绑定“编辑”和“删除”按钮的事件，分别调用 `PUT` 和 `DELETE` 接口，并在成功后刷新列表。
    - **预期改动（预判）**:
      - `lugarden_universal/public/admin.html` (为按钮添加事件监听器，实现调用 API 和更新 UI)
...
- [ ] **任务A.2：重构后台为“分模块、动态加载”架构**
  - [ ] **步骤 A.2.1:** 创建后台模块加载器的基础 `admin.js`，并将其链接到 `admin.html`。
    - **预期改动（预判）**:
      - `lugarden_universal/public/assets/admin.js` (新建，包含主逻辑、事件委托)
      - `lugarden_universal/public/admin.html` (移除所有内联 `<script>` 逻辑，添加 `<script src="assets/admin.js">`)
  - [ ] **步骤 A.2.2:** 实现动态模块加载机制，当在仪表盘点击某个宇宙的“管理”按钮时，能根据其 `type` 加载对应的 JS 模块。
    - **预期改动（预判）**:
      - `lugarden_universal/public/assets/admin.js` (实现动态 `import()` 逻辑，清空并渲染主内容区)
  - [ ] **步骤 A.2.3:** 创建“周与春秋”宇宙的管理模块，将原有的管理功能（子项目、诗歌、问答管理）封装进去。
    - **预期改动（预判）**:
      - `lugarden_universal/public/assets/universe-modules/zhou.js` (新建，封装旧的管理功能为可导出的函数)
      - `lugarden_universal/public/admin.html` (移除旧的管理界面静态 HTML，它们将由 `zhou.js` 模块动态生成)
  - [ ] **步骤 A.2.4:** 创建“毛小豆”宇宙管理模块的占位符文件和界面。
    - **预期改动（预判）**:
      - `lugarden_universal/public/assets/universe-modules/maoxiaodou.js` (新建，导出一个简单的渲染函数，显示“施工中”等信息)
      - `lugarden_universal/public/assets/admin.js` (确保能正确加载并渲染毛小豆模块的占位符界面)
  - **完成状态**: 🔄 进行中
