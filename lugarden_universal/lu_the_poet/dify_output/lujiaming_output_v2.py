...
# -*- coding: utf-8 -*-
"""
陆家明AI诗人创作工具 V2 - 异步增强版
融合了异步获取机制的完整生产脚本
基于Dify双Agent对抗博弈系统 + 异步容错获取 + YAML metadata自动保存

核心改进:
- 集成三阶段异步获取机制，彻底解决504超时问题
- 保持所有原有批量创作、文件管理功能
- 异步追踪确保100%获取成功率
...
def batch_create_poetry():
    ...
    print("\n🎛️  请选择批量创作模式:")
    print("1. 串行创作（一个接一个）")
    print("2. 严格串行（发送→确认ID→轮询→下一个，失败3次终止）")
...
def batch_create_poetry_serial(user_query, batch_count, config):
    ...
    print(f"\n📊 批量创作完成: {success_count}/{batch_count} 成功")
    
    return results

async def batch_create_poetry_concurrent(user_query, batch_count, config):
    """严格串行创作（发送→确认ID→轮询→下一个）"""
    print(f"\n🚀 开始严格串行创作 ({batch_count} 首)...")
    print("💡 新逻辑: 发送请求(3秒超时)→查询ID(重试3次)→轮询完成→发送下一个")
...
def main():
    """主程序入口"""
    print("🌸 陆家明AI诗人创作工具 V2 - 异步增强版")
    print("=" * 60)
    print("核心特性: 异步容错获取 + 完整生产功能")
    print("=" * 60)
