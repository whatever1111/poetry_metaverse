...
sed -i "s/DOMAIN=.*/DOMAIN=$DOMAIN/" ../application/.env

# 确保必要目录存在
echo "📁 创建必要目录..."
mkdir -p ../data ../public ssl
...
if docker-compose ps | grep -q "Up"; then
    ...
    echo ""
    echo "👤 管理员账号:"
    echo "   用户名: $(grep ADMIN_USERNAME ../application/.env | cut -d'=' -f2)"
echo "   密码: $(grep ADMIN_PASSWORD ../application/.env | cut -d'=' -f2)"
    echo ""
    echo "⚠️  重要提醒:"
